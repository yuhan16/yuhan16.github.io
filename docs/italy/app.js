...
import {STORAGE_KEY,ZONES,clone,uid,requireDay,selectedPlan,defaultPlanId,canDeletePlan,deletePlan,nextBackupLabel,hasTimeConflict,candidates,ensureLatest,applyPlan,updateItem,addItem,removeItem,moveItem,setStatus,safeUrl,mapUrl,validDate,validTime,localToUtc,beijingTime,serialize,deserialize,saveState} from './core.js';
...
function suggestedField(label,name,value,suggestions) {
  const hint=suggestions.join('、');
  return `<div class="suggested-field">${field(label,name,value,'text',`list="${name}-suggestions" placeholder="${escape(hint)}" aria-describedby="${name}-hint"`)}<p id="${name}-hint" class="field-hint">例如：${escape(hint)}，也可自行填写。</p><datalist id="${name}-suggestions">${suggestions.map(text=>`<option value="${escape(text)}"></option>`).join('')}</datalist></div>`;
}
...
function renderDay() {
  ...
  return `${dayHeading()}<div class="chips" aria-label="每日方案">${[...d.plans,...(d.latest?[d.latest]:[])].map(p=>action('plan',escape(p.label),`data-id="${escape(p.id)}" aria-pressed="${p.id===currentId()}"`,'chip '+(p.id===currentId()?'selected':''))).join('')}</div>
    <div class="toolbar"><span class="save-status">${saveLabel()}</span><div class="toolbar-actions">${action('save-plan','另存为备选','','small')}${canDeletePlan(d,currentId())?action('delete-plan','删除备选',`data-id="${escape(currentId())}"`,'small quiet danger'):''}${action('day-info','当天备注','','small quiet')}${action('view-candidates','＋ 加入项目','','small')}</div></div>
...
document.addEventListener('click',event=>{const nav=event.target.closest('[data-view]');if(nav){setView(nav.dataset.view);return;}const b=event.target.closest('[data-action]');if(!b||b.disabled)return;const id=b.dataset.id,a=b.dataset.action;
  try {
    ...
    if(a==='save-plan'){openDialog('另存为备选',`<p>保存当前查看的方案，之后可以随时切换查看。</p><form>${field('方案标签','label',nextBackupLabel(day()),'text','required maxlength="50"')}</form>`,action('confirm-save-plan','保存备选','','primary'));return;}
    if(a==='delete-plan'){
      const d=day();if(!canDeletePlan(d,id))return;const label=selectedPlan(d,id).label;
      if(!confirm(`删除【${label}】？地点库、已完成记录、固定安排和最新调整会保留。只出现在此备选中的未完成项目会退出当天候选。`))return;
      commit(s=>deletePlan(requireDay(s,dayId),id),`已删除【${label}】`);return;
    }
